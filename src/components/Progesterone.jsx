...
import { getProgesteroneForm } from "../utils/helpers.js";

export function ProgesteroneDashboard({ templates, onSelect, onBack }) {
    ...
    <section className="dashboard-shell">
      <header className="template-header compact">
        ...
        <div>
          ...
          <h1>Progesterone IUD Insertion</h1>
        </div>
        <FileText aria-hidden="true" className="header-symbol" />
      </header>

      <div className="selector-band">
        <label htmlFor="progesterone-template-select">Choose a progesterone template</label>
        <div className="selector-row">
          <select id="progesterone-template-select" value={choice} onChange={(event) => setChoice(event.target.value)}>
...
export function ProgesteroneTemplatePage({ template, onBack }) {
  const form = getProgesteroneForm(template);

  return (
    <section className="template-shell">
      <header className="template-header">
        <button type="button" className="icon-button" onClick={onBack} aria-label="Back to progesterone list">
          <ArrowLeft aria-hidden="true" />
        </button>
        <div>
          <p className="eyebrow">IUD Insertion Bundle</p>
